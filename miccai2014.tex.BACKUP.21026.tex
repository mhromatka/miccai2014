...
<<<<<<< HEAD
The multi-task learning approach achieved significantly better overall accuracy than the single task approach, which in turn is much better than the individual site approach. Using MTL, we classified the ABIDE data with an  overall accuracy of 66.3\%, with 68\% sensitivity and 64\% specificity.  Pooling all data resulted in 64.3\% overall accuracy, 68\% sensitivity and 60\% specificity. The summed overall accuracy of each Individual site was 58.1\% with sensitivity of 64\% and specificity of 53\%. 
=======
...
>>>>>>> 219ccd80d0efb24af99c5ddfb8460bc7d34e82bf
\begin{figure}
	\centering
...
<<<<<<< HEAD
Several sites benefited greatly from the increase in data size, most notably OHSU, KKI and MaxMun.  For sites that were relatively stable to begin with because of large individual site sample size, the MTL approach found little to no improvement to that sites overall accuracy, see NYU, UCLA, UM, and USM. This is an expected **** of multi-task learning, as the larger sites have enough data to design a competent classifier.  By requiring these sites to be near the combined site mean, a decrease in accuracy is likely as the decision boundary is deviating from what was already a stable result. 
...
\begin{figure}
	\centering
	\includegraphics[scale = .15]{now_circos2.png}
	\caption{The pairwise connections, represented as ribbons connecting each region, selected by the method described in \ref{subsec:FS}.  The width of each ribbon is determined by the weights from the $w_0$ vector used in the SVM. }
	\label{fig:circos}
\end{figure}
